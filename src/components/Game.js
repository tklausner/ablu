...
class Game extends React.Component {
  ...
  async checkWinner() {
    // let { active } = await getGameState("main");
    // if (active) {
    //   let tile = await getTile(this.state.wIndex);
    //   console.log("WINNER", tile.bids[0]);
    //   await updateUserBalance(tile.bids[0], this.state.pot);
    //   this.setState(() => ({
    //     loading: true,
    //     hoursMinSecs: {
    //       hours: 99,
  ...
  async componentDidMount() {
    ...
    // get game state
    let { pot, id, lub, memMap, endTime } = await getGameState('main');
    if (tiles.size <= 1) {
      await createTiles();
    }
    ...
    return !this.state.loading ? (
      <div>
        <div className="divRow">
          <Paper className={classes.paper} id="count">
            <p id="balContent">{this.state.players + ' ppl'}</p>
          </Paper>
          <Paper className={classes.paper} id="pot">
            <p id="balContent">{this.state.pot + '°'}</p>
          </Paper>
        ...
        <div className="divCol">
                ...
                {[0, 1, 2, 3].map((id) => (
                  <Grid key={id} item>
                    <Button
                      onClick={(e) => {
                        this.state.infoMode
                          ? this.infoBarPressed(id)
                          : this.buyBarPressed(id);
                      }}
                    >
                      <Paper
                        className={classes.paper}
                        style={{ backgroundColor: this.barBackground(id) }}
                      >
                        <p className="bar">{this.barContent(id)}</p>
                      </Paper>
                    </Button>
                  </Grid>
                ))}
          ...
          {!this.state.infoMode ? (
            <Paper className={classes.paper} id="aux">
              <p id="balContent">{this.state.user.balance + '°'}</p>
            </Paper>
          ) : null}
        </div>
      </div>
  ...
  switchState(arr) {
    if (this.state.playerMap[this.state.id]) {
      return this.state.id === this.state.wIndex
        ? arr[0]
        : this.state.board.get(this.state.id) === 1
        ? arr[1]
        : arr[2];
    } else {
      return arr[3];
  ...
  buyBarPressed(index) {
    ...
    switch (index) {
      case 1:
        if (
          !this.state.playerMap[this.state.id] &&
