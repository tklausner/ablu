  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '1%' }}>
      <form className={classes.root} noValidate autoComplete="off">
        <div>
          <TextField id="email-input" label="Email" variant="outlined" onChange={event => handleEmail(event.target.value)} />
        </div>
        <div>
          <TextField id="password-input" label="Password" variant="outlined" onChange={event => handlePassword(event.target.value)} type="" />
        </div>
        <div> {password.length > 3 && email.length > 3 ?
          <Button onClick={() => loginHelper(loginGame, email, password)} className={classes.button}>
